export let SampleInput : Array<string> = [
...
export function WhatDigit(segments: string, signal: { [key: string]: string }) : number {
    if (segments.length === 2) return 1;
    if (segments.length === 3) return 7;
    if (segments.length === 4) return 4;
    if (segments.length === 7) return 8;

    let segmentsorted = segments.split("").sort().join("");

    switch (segmentsorted) {
        case Digit(0, signal): return 0;
        case Digit(2, signal): return 2;
        case Digit(3, signal): return 3;
        case Digit(5, signal): return 5;
        case Digit(6, signal): return 6;
        case Digit(9, signal): return 9;
    }

    return 0;
}

function Digit(digit: number, signals: { [key: string]: string}) : string {
    switch (digit) {
        case 0: {
            return [signals[SegmentLocation.TopLeft] 
            , signals[SegmentLocation.Top]
            , signals[SegmentLocation.TopRight]
            , signals[SegmentLocation.BottomLeft]
            , signals[SegmentLocation.Bottom]
            , signals[SegmentLocation.BottomRight]].sort().join("");
        }
        case 2: {
            return [signals[SegmentLocation.Top]
            , signals[SegmentLocation.TopRight]
            , signals[SegmentLocation.Middle]
            , signals[SegmentLocation.BottomLeft]
            , signals[SegmentLocation.Bottom]].sort().join("");
        }
        case 3: {
            return [signals[SegmentLocation.Top]
            , signals[SegmentLocation.TopRight]
            , signals[SegmentLocation.Middle]
            , signals[SegmentLocation.BottomRight]
            , signals[SegmentLocation.Bottom]].sort().join("");
        }
        case 5: {
            return [signals[SegmentLocation.Top]
            , signals[SegmentLocation.TopLeft]
            , signals[SegmentLocation.Middle]
            , signals[SegmentLocation.BottomRight]
            , signals[SegmentLocation.Bottom]].sort().join("");
        }
